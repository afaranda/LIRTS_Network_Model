##############################################################################
# File: Pax6_Targets_Time_Series.R
# Purpose: Analyze the changes that occur in Pax6 targets in the 
#          wildtype time series. Tables of potential Runx1 targets were
#          were retrieved from the TRRUST database on Nov 4, 2020.
#          This script generates time series plots for the Pax6 
...
#          script (plots and tables) have the prefix "P6T" 
#         
...
runx1.hs<-read.table(
  paste(
    wd,
    "/LEC_Time_Series_HTSeq_Counts/PAX6_targets.human.tsv",
    sep=""
  )
)
...
runx1.mm<-read.table(
  paste(
    wd,
    "/LEC_Time_Series_HTSeq_Counts/Pax6_targets.mouse.tsv",
    sep=""
  )
)
...
for (g in names(sig.tgt)){
  if(g %in% deg$SYMBOL){
    ...
    ggsave(
      filename=paste(
        "LTS_DEG_Analysis_results/P6T_",g,
        "_fpkm_box.png", sep=""
      ), width=6, height=2
    )
...
ggplot(plt.all, aes(x=interval, y=FPKM, group=interval)) + 
  ...
  ggtitle("Pax6 targets in LEC Time Series") +
  facet_wrap( ~gene_symbol, scales="free") +
  theme(axis.text.x = element_text(angle=45))
ggsave(
  filename=paste(
    "LTS_DEG_Analysis_results/P6T_Pax6_target_LTS_PanelPlot.png",sep=""
  ), width=9, height=12
)
