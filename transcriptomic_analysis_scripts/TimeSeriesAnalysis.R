...
deg$MaxLFC<-apply(deg[,grep("logFC", names(deg))],1, function(x){
  m<-max(abs(x))
  for(i in 1:length(x)){
    if (abs(x[i]) == m){
      return (m * sign(x[i]))
...
deg$MaxLFC_Interval<-apply(deg[,grep("logFC", names(deg))],1, function(x){
  m<-max(abs(x))
  interval=c('6H', '24H', '48H', '120H')
  for(i in 1:length(x)){
    if (abs(x[i]) == m){
      return (interval[i])
    }
...
write.table(
  uniqueMaxLfc(deg, idc='SYMBOL', lfc="MaxLFC$"), 
  "DEG_Table_For_Phixer_plot.txt", row.names=F, 
  sep = '\t', quote=F
)
